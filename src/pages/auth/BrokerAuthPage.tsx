...
export const BrokerAuthPage: React.FC = () => {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      await signInWithGoogle('broker');
    } catch {
      setIsGoogleLoading(false);
    }
  };

  const handleBrokerSignUp = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!agreed) {
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 bg-white">
        <div className="w-full max-w-md">
          ...
          >
            ...
            {isGoogleLoading ? 'Redirecting to Google...' : 'Sign up as Broker with Google'}
          </button>
          ...
          <form className="space-y-4" onSubmit={handleBrokerSignUp}>
            <Input
              label="Full Name"
